...
## v15
- 热力图绿色深浅改为按“当天记录过的最高实际交易总额（CNY）”计算，不再按当天快照条数计算。
- 当前年份最高金额为最亮绿色基准，其余日期按相对金额自动分成 4 档；以后出现更高投入后，整年颜色会实时重新归一化。
- 历史记录现在同时写入 IndexedDB 和 localStorage，并在保存时尝试申请持久化存储。
- 新增“备份 / 导入”JSON 功能，用于换机、清理浏览器数据或重装后的手工恢复；浏览器本地存储本身无法保证绝对永久。
